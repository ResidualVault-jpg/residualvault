...
class VeoVideoAgent extends BaseAgent {
  ...
  async writeOfficeJourneyScript() {
    var raw = await this.ask(
      ...
      'Scene 4 (DESK): Bryan sits at his desk, looks at camera. Delivers top yields this week (up to ' + BRAND_STATS.topApy + ' APY) and CTA to visit ResidualVault.com.\n\n' +
  ...
  async generateSceneClips(scenes) {
    ...
    for (var i = 0; i < scenes.length; i++) {
      ...
      var op = await this._veoGenerate(prompt);
      if (op.error) {
  ...
  async renderFinalVideo(clips, title) {
    var completedClips = clips.filter(function(c) { return c.status === 'complete'; });
    if (completedClips.length === 0) throw new Error('No clips to render');

    // For now use the first clip - full multi-clip Remotion pipeline comes next
    var primaryClip = completedClips[0].path;
